...
- `bakemono-server-<target>.tar.gz` per server platform (Linux x64 + macOS arm64): the `bakemono` binary plus bundled gallery-dl / ffmpeg, so a host untars and runs the board from the console without Docker
